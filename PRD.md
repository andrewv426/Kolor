...
- True camera-RAW (.DNG/.CR2) editing, layers, masks, local adjustments, brushes.
...
2. **Fixed color space.** Tag the drawing buffer sRGB (context attributes pinned in §6.2.1); decode sRGB→linear at shader entry via the in-shader EOTF, do all tone math in linear light, re-encode via the in-shader OETF at output. sRGB *encoding* happens **only** in-shader — the canvas colorspace is just the display tag, never a second conversion.
...
All ten core ops (plus optional clarity) run **in linear light** (between EOTF and OETF), at `highp`, in this fixed, **non-commuting** order (restating §6.2 rule 3):

> **white balance (temp → tint) → exposure → contrast → highlights → shadows → whites → blacks → vibrance → saturation → (clarity)**
...
##### Frozen per-op formulas + integer→amount mapping (v1, linear light)

These exact mappings make "v1 freezes the slider→math mapping" (§6.2 rule 1, freeze rule 5) concrete. Each slider value `s ∈ [-100, +100]` (integer) is normalized to `n = s / 100.0 ∈ [-1, 1]`. All ops run on linear-light RGB `c = (r, g, b)` (post-EOTF), at `highp`, in the frozen op order above. **Luminance** uses Rec. 709 linear weights: `Y = dot(c, vec3(0.2126, 0.7152, 0.0722))`. Every constant below is frozen; changing any one ships as v2.

1. **White balance — temperature (`temp`).** Per-channel linear gain on a warm↔cool axis: `r *= 1 + 0.20*n; b *= 1 - 0.20*n` (`g` unchanged). `n = temp/100`.
2. **White balance — tint (`tint`).** Green↔magenta axis: `g *= 1 - 0.10*n; r *= 1 + 0.05*n; b *= 1 + 0.05*n`. `n = tint/100`. (Applied with temp as the single "white balance" op, temp first.)
3. **Exposure.** Linear gain of `±2` stops: `c *= exp2(2.0 * n)`, i.e. `c *= 2^(2*exposure/100)`. `n = exposure/100`.
4. **Contrast.** Around a fixed **linear-light pivot `P = 0.18`** (mid-gray): `c = (c - P) * (1 + n) + P`, with `n = contrast/100` (slope range `[0, 2]`). **Then pin the negative lobe: `c = max(c, 0)`** (frozen). Contrast can drive dark channels below 0; this pin is required *before* the luma/chroma-dependent ops 5–9 so their masks and ratios never see a negative channel (see vibrance, op 9, for the failure mode it prevents).
5. **Highlights.** Brighten/recover upper tones via a luminance mask `wH = smoothstep(0.5, 1.0, Y)`: `c *= 1 + 0.5 * n * wH`. `n = highlights/100`.
6. **Shadows.** Lift/deepen lower tones via `wS = smoothstep(0.5, 0.0, Y)` (i.e. `1 - smoothstep(0.0, 0.5, Y)`): `c *= 1 + 0.5 * n * wS`. `n = shadows/100`.
7. **Whites.** Move the white point: `c *= 1 + 0.25 * n`. `n = whites/100`.
8. **Blacks.** Move the black point with a near-black mask `wB = clamp(1 - Y/0.25, 0, 1)`: `c += 0.10 * n * wB`. `n = blacks/100`. **Then pin the negative lobe: `c = max(c, 0)`** (frozen). Blacks with negative `n` is additive and can push dark channels below 0; this pin restores non-negativity before vibrance (op 9). (Ops 1–3 and 5–7 are strictly positive-preserving on non-negative input — temp/tint scale by `1 ± 0.20n` etc. with coefficient < 1, exposure by `exp2(·) > 0`, highlights/shadows by `1 + 0.5·n·w ≥ 0.5`, whites by `1 + 0.25n ≥ 0.75` — so only ops 4 and 8 need a pin.)
9. **Vibrance** (skin-protected, low-saturation-weighted). Let `mx = max(r,g,b)`, `mn = min(r,g,b)`, **`sat = clamp((mx - mn) / max(mx, 1e-5), 0, 1)`** (current saturation ∈ `[0,1]`; the **clamp to `[0,1]` is frozen**). Weight `w = 1 - sat` (less-saturated pixels get boosted more) **times a skin guard** `g_skin = 1 - 0.5 * exp(-pow((hue - 25°)/20°, 2))` computed from the pixel hue (protects orange/skin hues): `c = mix(vec3(Y), c, 1 + 0.5 * n * w * g_skin)`. `n = vibrance/100`. (Hue from linear RGB → HSV; the `25°/20°` Gaussian and `0.5` depth are frozen.) **Why the clamp + the op-4/op-8 pins matter:** without the upstream pins a negative `mx` collapses the denominator to `1e-5` while `(mx - mn)` stays positive, exploding `sat` to ~1e2–1e5, flipping the mix factor hugely negative, and inverting dark pixels into saturated pure primaries — the red/green/blue speckle. The pins keep `mx ≥ mn ≥ 0` so `sat ≤ 1` already; the `clamp` here is belt-and-braces.
10. **Saturation** (uniform). `c = mix(vec3(Y), c, 1 + n)`, so `n = -1` (saturation `-100`) → grayscale, `n = +1` → 2× saturation. `n = saturation/100`.
11. **Clarity** (optional, last). Local midtone contrast: `c = c + 0.30 * n * (c - blur(c)) * wM`, where `blur` is a fixed-radius Gaussian on the linear-light image and `wM = 1 - abs(2*Y - 1)` masks to midtones. `n = clarity/100`. The blur kernel radius/weights are frozen with v1; if clarity is cut, this op is identity.

After op 10 (or 11), `c` is clamped to `[0, 1]`, OETF-encoded (`linearToSrgb`), dithered, and quantized (below). These formulas — pivots, gains, mask curves, weights, and constants — are **frozen**; a v1 conformance render of a known settings vector against a known master16 swatch is checked in CI alongside the decode swatch test.
...
1. The **canonical source** definition (2048px 16-bit sRGB-encoded PNG master) and the **variant set** — members (master16/preview8/ai768), resolutions, bit depths, formats, pinned encoders/params, and which surface consumes each.
2. The **client decode + texture path**: the pinned context attributes (`alpha:false`, `premultipliedAlpha:false`, `preserveDrawingBuffer:false`, `antialias:false`, `drawingBufferColorSpace:'srgb'`), UPNG.js pinned version, the big-endian `(hi<<8)|lo` sample reassembly + RGBA channel order, the `s/65535 →` half-float **bit-pattern packer** (and its rounding) → `RGBA16F`, the pinned unpack state (`UNPACK_FLIP_Y_WEBGL = false`, `PREMULTIPLY_ALPHA = false`, `COLORSPACE_CONVERSION = NONE`) and forced-opaque alpha, `LINEAR` filtering, sample-then-decode (no hardware sRGB sampling), the **canonical 2048px render resolution** for exact-settings renders; and the 3-tier fallback ladder + its capability checks.
...
**Amendment (2026-06-12, pre-launch) — negative-lobe pinning + clamped vibrance `sat`.** The op-4 and op-8 `c = max(c, 0)` pins and the `clamp(…, 0, 1)` on vibrance `sat` (above) were added to fix a numerical instability: contrast/blacks could push dark channels negative, collapsing the vibrance saturation denominator and inverting dark pixels into saturated red/green/blue speckle (worst with vibrance, also visible under contrast/blacks/saturation). This was amended **in place under `v1`** — not minted as `v2` — **only because zero user edits had been minted yet** (pre-launch). The original formulas without the pins would otherwise be frozen forever; correcting them now avoids permanently enshrining a numerically unstable renderer. **This in-place exception is pre-launch only:** once any real edit exists, any such change MUST ship as `v2` per the freeze rule above. The CI conformance render (below) is updated to the pinned outputs.
...
### 6.8 Photo pipeline
- **"Raw" = the canonical preprocessed master (§6.2.1), not camera RAW.** The master is *derived from* a high-quality **unedited JPEG** (the delivered stock photo), then preprocessed into the 16-bit master16 + variant set so the sliders have real tonal headroom — JPEGs alone clip highlights and band under exposure/contrast (§6.2.1). True RAW (.DNG/.CR2) needs LibRaw-WASM, ~15–20s/file decode, 20–50MB files, and licensing gaps — fatal for a snappy daily game, and no stock API even serves it. **UI wording: call it "unedited"/"unstyled," not "RAW."**
...
- **Store the canonical master + its variant set once/day** (§6.2.1): master16 (~8–18MB PNG16) + preview8 (small WebP) + ai768 (small JPEG) in Supabase Storage, plus the preprocessing manifest. ~10–20MB/day total — still trivial.
...
- **Result:** storage ≈ one master + variants/day + kilobytes of JSON — comfortably inside Supabase's free tier for a long time. Egress: the only weighty asset is master16, but it's **one immutable URL/day** served through a free CDN layer (Cloudflare in front of Supabase Storage → near-100% cache hit, so origin egress stays near zero regardless of player count); the gallery and landing hero load only the small **preview8**, and the editor fetches master16 once per day. Trade-off: requires the deterministic versioned pipeline (§6.2 / §6.2.1) — which we want anyway for "inspect exact settings" and AI players.
...
## 9. Cost
- **$0/mo for MVP and early traction** (Supabase free: 500MB DB / 1GB storage / 5GB egress / 50k MAU / 500k edge fns. The settings-only design barely touches DB. Storage is ~10–20MB/day of masters+variants — still trivial against the 1GB cap for a long time, and prunable. Egress is the one line to watch: master16 is ~8–18MB, so serving it from Supabase origin to every editor session *would* blow the 5GB/mo free egress quickly — which is exactly why master16 is **one immutable URL/day fronted by a free CDN (Cloudflare)** with near-100% cache hit, keeping origin egress near zero; the gallery/landing only ever pull the small preview8. Pexels free; Vercel Hobby $0 but non-commercial.) ⚠️ Keep the free Supabase project alive with a daily cron ping (it pauses after a week idle).
