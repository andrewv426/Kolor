...
4. **Contrast.** Around a fixed **linear-light pivot `P = 0.18`** (mid-gray): `c = (c - P) * (1 + n) + P`, with `n = contrast/100` (slope range `[0, 2]`).
...
8. **Blacks.** Move the black point with a near-black mask `wB = clamp(1 - Y/0.25, 0, 1)`: `c += 0.10 * n * wB`. `n = blacks/100`.
9. **Vibrance** (skin-protected, low-saturation-weighted). Let `mx = max(r,g,b)`, `mn = min(r,g,b)`, `sat = (mx - mn) / max(mx, 1e-5)` (current saturation ∈ `[0,1]`). Weight `w = 1 - sat` (less-saturated pixels get boosted more) **times a skin guard** `g_skin = 1 - 0.5 * exp(-pow((hue - 25°)/20°, 2))` computed from the pixel hue (protects orange/skin hues): `c = mix(vec3(Y), c, 1 + 0.5 * n * w * g_skin)`. `n = vibrance/100`. (Hue from linear RGB → HSV; the `25°/20°` Gaussian and `0.5` depth are frozen.)
