...
- **Serve the photo variants as immutable, long-lived cache entries.** Each staged variant (`master16`/`-hi`/`-lo`, `preview8`, `ai768`, `manifest.json`) is content-frozen once shipped (§6.2.1), so it must be served with `Cache-Control: public, max-age=31536000, immutable` — a returning player should re-use the ~7.8MB two-plane master from cache with **zero network**, never a revalidation round-trip. *Local mode:* `next.config.ts` sets this for `/public/photo/:path*` (Next/Vercel's default for `/public` is `max-age=0, must-revalidate`, which forces a 304 on every load — the original "slow to cache in" symptom). *Production:* the Vercel header rule does **not** cover Supabase-Storage URLs (different origin; Supabase default is only `max-age=3600`) — set `cacheControl: '31536000, immutable'` at **upload** in `scripts/prepare-master` / the pre-stage workflow. **Load-bearing safety invariant:** variant filenames are fixed-per-variant and **not** content-hashed (uniqueness is only the `<id>`/path segment), so `immutable` is correct ONLY because a shipped `/photo/<id>/<file>` is never overwritten — **a re-stage MUST use a new id / new `daily_photos` row, never an in-place overwrite** (otherwise warm caches serve stale bytes for up to a year). The pre-stage model (a new staged photo per day) already satisfies this.
