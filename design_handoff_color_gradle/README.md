...
- **Phone:** Photo pinned top (~46vh band) with a live preview, shown in full (`contain`) so landscape photos letterbox within the band rather than cropping. Top bar: "‹ Exit", `Day {n}` badge, **timer** (mono; turns accent/`.warn` at ≤60s). A "Tap to compare" ghost button (press-and-hold reveals the unedited original) bottom-left over the photo. Below: scrollable list of all 10 labelled sliders; "Adjust" eyebrow + "10 sliders". Sticky bottom bar: "Reset" (ghost, zeros **all** 10) + "Submit" (primary, fills width).
- **Desktop:** Photo left on a recessed `--bg-2` letterbox. The stage now **adapts to the daily photo's aspect ratio** (set from its width/height) so portrait AND landscape photos display in full — no crop, no distortion — replacing the previous fixed 4:5 frame. Capped at max-width 640px and max-height 80dvh, centered in the letterbox. Right: a 360px panel — header "Adjust" + "10 sliders", scrollable slider list, footer "Reset" (zeros **all** 10) + "Submit edit".
- **Per-slider reset:** every editor slider has its own reset — a small circular-arrow (↺) icon next to the value readout that zeros **just that slider**. It's dimmed + disabled while the slider is already at 0 (nothing to reset) and brightens once moved, doubling as a "modified" cue. Double-clicking the track still resets a slider too; the footer "Reset" zeros all 10. (Reset icons appear only in the editor — the read-only Inspect recipe never shows them.)
- **Compare button label:** "Tap to compare" on phone, "Hold to compare" on desktop; shows "Before" while held. The press-and-hold is **pointer-capture based**, so the hold survives the button's label-width change and a cursor that drifts off the button (it no longer snaps off mid-hold); release returns to the edited view.
- **Photo-loading state:** the photo is visible almost immediately — the editor paints the tiny `preview8` variant (~95KB) as a **blurred placeholder** (slightly over-scaled to hide its lower resolution) the instant it arrives, rather than holding an empty letterbox until the multi-MB master finishes. Over the (blurred) photo, a centered loading indicator shows: a sentence-case mono label "Loading master — 43%" (`--ink-2`, DM Mono, tnum) above a thin amber (`--accent`) progress bar on a hairline (`--line`) track, ~220px wide. A faint top scrim gradient (replacing the old opaque `--bg-2` letterbox) keeps the label legible on bright photos. The percentage is driven by real download bytes; once the master decodes, the sharp WebGL canvas **cross-fades in** over the placeholder (~0.32s) and the placeholder is dropped. Minimal, no spinner; both the bar's width transition and the canvas cross-fade are gated by `prefers-reduced-motion` (which snaps the canvas in instantly). The placeholder is **editor-only** (it rides the `showProgress` opt-in) and is purely a loading affordance — it never feeds the renderer, so it doesn't affect the deterministic render. (The master is delivered as two lossless-WebP planes ≈42% the size of the source PNG — see PRD §6.2.1 delivery amendment.)
